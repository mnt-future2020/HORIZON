...
const VenueOwnerDashboard = lazyRetry(
  () => import("@/pages/VenueOwnerDashboard"),
);
...
const SuperAdminDashboard = lazyRetry(
...
const SharedHighlightPage = lazyRetry(
  () => import("@/pages/SharedHighlightPage"),
);
...
const PrivacySettingsPage = lazyRetry(
...
const TournamentDetailPage = lazyRetry(
...
function ProtectedRoute({ children, roles }) {
  ...
  if (
    ...
    (user.doc_verification_status || "not_uploaded") !== "verified"
  ) {
    const path = window.location.pathname;
    const allowed = VENUE_OWNER_ALLOWED_PATHS.some(
...
function DocVerificationPopup() {
  ...
  useEffect(() => {
    if (user && user.role === "venue_owner" && docStatus !== "verified")
      setOpen(true);
  }, [user, docStatus]);
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {docStatus === "rejected" ? (
              <>
                <XCircle className="h-5 w-5 text-destructive" /> Documents
                Rejected
              </>
            ) : docStatus === "pending_review" ? (
              <>
                <Clock className="h-5 w-5 text-amber-400" /> Under Review
              </>
            ) : (
              <>
          ...
          {docStatus === "not_uploaded" && (
            <div className="rounded-lg bg-blue-500/10 border border-blue-500/30 p-3 space-y-2">
              <p className="text-sm text-muted-foreground">
                ...
                and unlock full venue management features.
              </p>
          ...
          {docStatus === "pending_review" && (
            <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 p-3">
              <p className="text-sm text-muted-foreground">
                ...
                team. We'll notify you once verified.
              </p>
            </div>
          )}
          {docStatus === "rejected" && (
            <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-3 space-y-2">
              <p className="text-sm text-muted-foreground">
                Your documents were rejected. Please re-upload correct
                documents.
              </p>
              {user?.doc_rejection_reason && (
                <p className="text-xs text-destructive font-medium">
                  Reason: {user.doc_rejection_reason}
                </p>
              )}
            </div>
          )}
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          <ProtectedRoute roles={["venue_owner"]}>
            <VenueOwnerDashboard defaultView="manage" />
          </ProtectedRoute>
        }
      />
      <Route
      ...
      <Route
        ...
        element={
          <ProtectedRoute roles={["coach"]}>
            <CoachDashboard defaultView="coach_mgmt" />
          </ProtectedRoute>
        }
      />
      <Route
      ...
      />
      <Route
        path="/highlights/shared/:shareId"
        element={<SharedHighlightPage />}
      />
      <Route
        path="/iot"
        element={
          <ProtectedRoute roles={["venue_owner", "super_admin"]}>
            <IoTDashboard />
          </ProtectedRoute>
        }
      />
