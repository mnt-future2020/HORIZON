...
export default function VenueDiscovery() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-brand-600 selection:text-white" data-testid="venue-discovery-page">
      ...
      {!user && (
        <nav className="fixed top-0 w-full z-40 h-16 flex items-center justify-between px-6 md:px-12 bg-card/80 backdrop-blur-xl border-b border-border/20 shadow-sm">
          ...
          <div className="flex items-center gap-6">
            <Button variant="link" size="sm" onClick={() => navigate("/auth")} className="text-[11px] font-black uppercase tracking-widest text-muted-foreground hover:text-brand-600">Log in</Button>
        ...
        <div className="max-w-[90rem] mx-auto px-6 py-8">
          ...
          <div className="flex gap-4 items-center" data-testid="search-bar">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-16 pr-12 bg-secondary/20 border border-border/40 rounded-full h-14 text-lg font-bold focus-visible:ring-0 focus-visible:border-brand-500 placeholder:text-muted-foreground placeholder:font-medium transition-all"
            ...
            <Button variant="outline" size="icon" className={`h-14 w-14 shrink-0 relative rounded-full border transition-all ${filtersOpen ? "border-brand-600 bg-brand-50 text-brand-700" : "border-border/40 text-muted-foreground hover:text-brand-600 hover:border-brand-400"}`}
          ...
          <div className="flex gap-3 mt-6 overflow-x-auto pb-2 scrollbar-hide" data-testid="city-pills">
            <button onClick={handleNearMe} data-testid="near-me-btn"
              ...
              className={`px-5 py-2 text-[11px] font-black uppercase tracking-[0.15em] whitespace-nowrap transition-all flex items-center gap-2 admin-btn rounded-full ${nearMeActive ? "bg-brand-600 text-white shadow-md shadow-brand-600/20 border border-brand-600" : "bg-card border border-border/40 text-muted-foreground hover:text-foreground hover:border-border"}`}>
            ...
            {nearMeActive && (
              <button onClick={() => setDriveTimeMode(!driveTimeMode)} data-testid="drive-time-toggle"
                className={`px-5 py-2 text-[11px] font-black uppercase tracking-[0.15em] whitespace-nowrap transition-all flex items-center gap-2 admin-btn rounded-full ${driveTimeMode ? "bg-brand-600 text-white shadow-md shadow-brand-600/20 border border-brand-600" : "bg-card border border-border/40 text-muted-foreground hover:text-foreground hover:border-border"}`}>
            ...
            <button onClick={() => { setSelectedCity("all"); setNearMeActive(false); setUserLocation(null); setDistanceMap({}); setDriveTimeMap({}); setDriveTimeMode(false); }} data-testid="city-pill-all"
              className={`px-5 py-2 text-[11px] font-black uppercase tracking-[0.15em] whitespace-nowrap transition-all admin-btn rounded-full ${selectedCity === "all" && !nearMeActive ? "bg-brand-600 text-white shadow-md shadow-brand-600/20 border border-brand-600" : "bg-card border border-border/40 text-muted-foreground hover:text-foreground hover:border-border"}`}>
              All Cities
            </button>
            {cities.map(c => (
              <button key={c.city} onClick={() => { setSelectedCity(c.city); setNearMeActive(false); setUserLocation(null); setDistanceMap({}); }} data-testid={`city-pill-${c.city}`}
                className={`px-5 py-2 text-[11px] font-black uppercase tracking-[0.15em] whitespace-nowrap transition-all admin-btn rounded-full ${selectedCity === c.city && !nearMeActive ? "bg-brand-600 text-white shadow-md shadow-brand-600/20 border border-brand-600" : "bg-card border border-border/40 text-muted-foreground hover:text-foreground hover:border-border"}`}>
            ...
            {filtersOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 mt-6 border-t border-border/40">
                  {/* Area */}
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-2 block admin-section-label">Area</label>
                    <Select value={selectedArea} onValueChange={setSelectedArea}>
                      <SelectTrigger className="h-12 text-sm font-bold bg-secondary border border-border/40 rounded-xl focus:ring-0 focus:border-brand-500" data-testid="area-filter">
                        <SelectValue placeholder="All Areas" />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border border-border/40 shadow-lg">
                  ...
                  {/* Sport */}
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-2 block admin-section-label">Sport</label>
                    <Select value={selectedSport} onValueChange={setSelectedSport}>
                      <SelectTrigger className="h-12 text-sm font-bold bg-secondary border border-border/40 rounded-xl focus:ring-0 focus:border-brand-500" data-testid="sport-filter">
                  ...
                  {/* Price Range */}
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-2 block admin-section-label">Price</label>
                    <Select value={priceRange} onValueChange={setPriceRange}>
                      <SelectTrigger className="h-12 text-sm font-bold bg-secondary border border-border/40 rounded-xl focus:ring-0 focus:border-brand-500" data-testid="price-filter">
                  ...
                  {/* Amenity */}
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-2 block admin-section-label">Amenity</label>
                    <Select value={selectedAmenity} onValueChange={setSelectedAmenity}>
                      <SelectTrigger className="h-12 text-sm font-bold bg-secondary border border-border/40 rounded-xl focus:ring-0 focus:border-brand-500" data-testid="amenity-filter">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-6 pt-6 border-t border-border/40 gap-4">
                  <div className="flex items-center gap-4">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground admin-section-label">Sort By</label>
                    <Select value={sortBy} onValueChange={setSortBy}>
                      <SelectTrigger className="h-10 w-[160px] text-xs font-bold bg-secondary border border-border/40 rounded-xl focus:ring-0 focus:border-brand-500" data-testid="sort-select">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border border-border/40 shadow-lg">
                  ...
                  {activeFilterCount > 0 && (
                    <Button variant="ghost" size="sm" onClick={clearFilters} className="text-[11px] font-black uppercase tracking-widest text-muted-foreground hover:text-brand-600 hover:bg-brand-50 rounded-full h-10 px-4"
      ...
      <div className={`max-w-[90rem] mx-auto px-6 py-12 ${!user ? "pt-12" : ""}`}>
        <div className="flex items-center justify-between mb-8">
          <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest admin-section-label" data-testid="results-count">
            <span className="font-black text-foreground">{venues.length}</span> venue{venues.length !== 1 ? "s" : ""} found
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
              <div key={i} className="bg-card rounded-[28px] h-80 animate-pulse border border-border/40 shadow-sm" />
            ))}
          </div>
        ) : venues.length === 0 ? (
          <div className="bg-card rounded-[28px] border border-border/40 shadow-sm p-7 overflow-hidden" data-testid="no-results">
            <div className="grid md:grid-cols-2 gap-0">
              <div className="p-12 md:p-24 flex flex-col items-center md:items-start justify-center text-center md:text-left">
                <div className="w-16 h-16 bg-secondary rounded-[28px] flex items-center justify-center mb-6">
                  <Search className="h-6 w-6 text-muted-foreground" />
                </div>
                <p className="admin-page-title text-3xl mb-4 text-foreground">Zero Results</p>
                <p className="text-sm text-muted-foreground font-bold mb-8 uppercase tracking-widest leading-relaxed">Adjust your filters or broaden your search criteria to discover facilities.</p>
                <Button
                  className="bg-brand-600 text-white hover:bg-brand-700 rounded-full h-14 shadow-md shadow-brand-600/20 px-8 font-black uppercase tracking-[0.1em] text-sm transition-all"
            ...
            <AnimatePresence mode="popLayout">
              {venues.map((venue, idx) => (
                <motion.div key={venue.id} layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  className="group cursor-pointer flex flex-col h-full bg-card border border-border/40 hover:border-border/60 rounded-[28px] shadow-sm hover:shadow-md transition-all duration-300"
                  data-testid={`venue-card-${venue.id}`}>
                  {/* Image */}
                  <div className="relative aspect-[4/3] overflow-hidden bg-secondary/50 p-2">
                    ...
                    {(distanceMap[venue.id] != null || venue.distance_km != null) && (
                      <div className="absolute top-4 left-4 flex gap-2" data-testid={`distance-badge-${venue.id}`}>
                        <div className="bg-card/90 text-foreground px-3 py-1.5 text-[10px] font-black uppercase tracking-widest flex items-center shadow-md rounded-full">
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="font-medium text-lg text-foreground truncate group-hover:text-brand-600 transition-colors duration-300"
                    ...
                    {/* Location */}
                    <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-5">
                    ...
                    <div className="mt-auto flex items-center justify-between pt-4 border-t border-border/20">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-black text-sm text-foreground">{venue.rating?.toFixed(1) || "NEW"}</span>
                        {venue.total_reviews > 0 && (
                          <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest ml-1">({venue.total_reviews})</span>
                        )}
                      </div>

                      <div className="font-black text-lg text-brand-600">
                        ₹{venue.base_price || venue.price_per_hour}
                        <span className="text-[10px] text-muted-foreground uppercase tracking-widest ml-1">/HR</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
