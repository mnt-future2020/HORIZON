          ...
          >
            {contactInfo.map(({ icon: Icon, label, value, href }, idx) => (
              <motion.div
                ...
                className="rounded-2xl p-6 flex items-start gap-5 bg-gray-50 border border-gray-200 hover:border-brand-600/30 transition-all group"
              >
                ...
                <div>
                  <div className="text-xs text-gray-400 font-black uppercase tracking-[0.2em] mb-1.5">{label}</div>
                  {href ? (
                    <a href={href} className="text-base font-bold text-gray-900 hover:text-brand-600 transition-colors">{value}</a>
                  ) : (
            ...
            <motion.div
              ...
              className="rounded-2xl p-6 bg-gray-50 border border-gray-200"
            >
              <div className="text-xs text-gray-400 font-black uppercase tracking-[0.2em] mb-2">Legal Entity</div>
              <div className="text-base text-gray-900 font-bold">Magizh NexGen Technologies</div>
              <div className="text-sm text-gray-400 mt-1 font-medium">GST / CIN details available on request</div>
            </motion.div>
          </motion.div>
            ...
            {submitted ? (
              <div className="rounded-2xl p-12 flex flex-col items-center justify-center text-center gap-5 bg-gray-50 border border-gray-200 min-h-[500px]">
                ...
                <button
                  ...
                  className="mt-4 px-8 py-3 rounded-full border border-gray-300 text-gray-700 text-sm font-bold hover:border-brand-600/50 transition-colors"
                >
                  Send Another
                </button>
              </div>
              ...
              <div className="rounded-2xl p-8 sm:p-10 bg-gray-50 border border-gray-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input
                        ...
                        className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-5 py-3.5 text-base text-gray-900 font-medium placeholder:text-gray-400 focus:outline-none focus:border-brand-600/50 focus:ring-1 focus:ring-brand-600/30 transition-all"
                      />
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 font-black uppercase tracking-[0.15em]">Email *</label>
                      <input
                        type="email"
                        value={form.email}
                  ...
                  <div>
                    <label className="text-xs text-gray-500 font-black uppercase tracking-[0.15em]">Subject</label>
                    <input
                      value={form.subject}
                  ...
                  <div>
                    <label className="text-xs text-gray-500 font-black uppercase tracking-[0.15em]">Message *</label>
                    <textarea
                      value={form.message}
                      onChange={(e) => setForm((p) => ({ ...p, message: e.target.value }))}
                      placeholder="Describe your issue in detail..."
                      rows={5}
                      className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-5 py-3.5 text-base text-gray-900 font-medium placeholder:text-gray-400 resize-none focus:outline-none focus:border-brand-600/50 focus:ring-1 focus:ring-brand-600/30 transition-all"
                    />
                  </div>
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-brand-600 text-white font-black text-sm uppercase tracking-wider rounded-xl hover:bg-brand-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {submitting ? (
