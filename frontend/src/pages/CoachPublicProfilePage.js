...
function StatPill({ icon: Icon, label, value, color = "text-brand-600" }) {
  return (
    <div className="rounded-[28px] bg-card border border-border/40 shadow-sm p-4 text-center flex-1 min-w-[72px]">
...
function SectionCard({ icon: Icon, iconColor = "text-brand-600", title, children }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className="rounded-[28px] bg-card border border-border/40 shadow-sm p-6 space-y-3">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        {/* Cover strip */}
        <div className="h-20 bg-gradient-to-r from-brand-600/30 via-brand-600/10 to-transparent" />
        <div className="px-5 pb-5 -mt-10">
          <div className="flex items-end gap-4 mb-3">
            ...
            <div className="flex-1 min-w-0 pb-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-medium text-xl truncate">{coach.name}</h1>
                {coach.is_verified && (
            ...
            <div className="text-right shrink-0 pb-1">
              <p className="font-bold text-xl text-brand-600">₹{coach.session_price || 500}</p>
          ...
          <div className="flex gap-2">
            <Button className="flex-1 bg-brand-600 hover:bg-brand-500 text-white admin-btn rounded-xl shadow-lg shadow-brand-600/20 active:scale-[0.98] transition-all"
              onClick={() => navigate("/coaching")}>
              <Calendar className="h-4 w-4 mr-1.5" /> Book a Session
            </Button>
            <Button variant="outline" className="font-medium flex-1 border-brand-600/40 text-brand-600 hover:bg-brand-600/10 rounded-xl"
              onClick={() => navigate("/coaching")}>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        ...
        {yearsExp > 0 && (
          <StatPill icon={Briefcase} label="Yrs Exp" value={`${yearsExp}+`} color="text-brand-400" />
        )}
      </div>
        ...
        <SectionCard icon={GraduationCap} title="Specializations">
          <div className="flex flex-wrap gap-2">
            {coach.specializations.map((s, i) => (
              <Badge key={i} className="bg-brand-600/10 text-brand-600 border border-brand-600/20 text-xs admin-btn">
                {s}
              </Badge>
            ))}
          </div>
        ...
        <SectionCard icon={Trophy} iconColor="text-amber-500" title="Achievements">
          <div className="space-y-2">
            {coach.achievements.map((a, i) => (
              <CredentialItem key={i} icon={CheckCircle2} iconColor="text-brand-600"
      ...
      {coach.playing_history && (
        <SectionCard icon={Briefcase} iconColor="text-brand-400" title="Playing History">
            ...
            {Object.entries(slotsByDay).map(([dayIdx, slots]) => (
              <div key={dayIdx} className="flex items-center gap-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {slots.map((s, i) => (
                    <span key={i} className="text-[11px] admin-btn px-2 py-0.5 rounded-full bg-brand-600/10 text-brand-600 border border-brand-600/20">
      ...
      {packages.length > 0 && (
        <SectionCard icon={Package} title="Coaching Packages">
          <div className="space-y-3">
            {packages.map(pkg => (
              <div key={pkg.id} className="rounded-[28px] border border-border/40 bg-secondary/20 shadow-sm p-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {pkg.features?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {pkg.features.map(f => (
                        <span key={f} className="text-[10px] admin-btn px-1.5 py-0.5 rounded-full bg-brand-600/10 text-brand-600 border border-brand-600/20">✓ {f}</span>
                      ))}
                    </div>
                  )}
                </div>
          ...
          <Button className="w-full bg-brand-600 hover:bg-brand-500 text-white admin-btn rounded-xl shadow-lg shadow-brand-600/20 active:scale-[0.98] transition-all mt-1"
            onClick={() => navigate("/coaching")}>
      ...
      {/* ── Reviews ── */}
      {avgRating > 0 && (
        <div className="rounded-[28px] bg-card border border-border/40 shadow-sm p-6 text-center">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
        ...
        <Button className="w-full bg-brand-600 hover:bg-brand-500 text-white admin-btn rounded-xl shadow-lg shadow-brand-600/20 active:scale-[0.98] transition-all h-12 text-base"
          onClick={() => navigate("/coaching")}>
