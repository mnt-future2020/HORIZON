...
export default function BookingReceipt({ booking, compact }) {
  ...
  return (
    <div
      className={`rounded-2xl border border-border/50 bg-card/80 backdrop-blur-md ${compact ? "p-4" : "p-6"} space-y-4`}
    >
      {/* Header */}
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h3
            className={`font-display font-black text-foreground truncate ${compact ? "text-sm" : "text-lg"}`}
          >
            {booking.venue_name}
          </h3>
          {booking.turf_name && (
            <p className="text-xs text-muted-foreground font-semibold mt-0.5">
              {booking.turf_name} · {booking.sport}
            </p>
          )}
        </div>
        <Badge
          className={`${statusColor} uppercase text-[10px] font-bold border shrink-0`}
        >
